...
func solution크레인_인형뽑기_게임_2(_ board:[[Int]], _ moves:[Int]) -> Int {
    ...
    board.reversed().forEach{
        $0.enumerated().forEach{                        // $0.enumerated() -> index && value 같이 얻을 수 있음
            if $0.1 != 0 {                              //value가 0이 아니면 stack에 넣기
    ...
    moves.forEach{
        if let doll = stack[$0-1].popLast() {           // 꺼내는 인형을 doll에 저장
            if let last = bucket.last, last == doll {   // bucket의 마지막 인형 last에 저장 후 둘이 비교
                bucket.removeLast()                     //같으면 bucket마지막 인형 제거 -> 2개가 제거됨 -> anwer += 2
                answer += 2
            } else {                                    //다르면 bucket에 넣어주기
                bucket.append(doll)
            }
        }
    }
